...
function githubRequest(sourceEvent, payload, options = {}) {
  return handleGitHub({
    headers: { "x-github-event": sourceEvent },
...
test("handleGitHub acknowledges supported events and accepts base64 bodies", () => {
  const response = githubRequest(
    "issues", { action: "opened", repository, sender: user, issue },
    { isBase64Encoded: false },
  );
  const base64Response = handleGitHub({
    headers: { "X-GitHub-Event": "issues" },
    body: Buffer.from(
      JSON.stringify({ action: "opened", repository, sender: user, issue }),
    ).toString("base64"),
...
test("supported deliveries reject malformed common and nested payload fields", () => {
  ...
  const invalidIssues = [
    null, [], {}, { ...base, issue, action: 3 },
    { ...base, issue, repository: invalidRepository }, { ...base, issue, sender: invalidSender },
    { ...base, issue: missingTitle }, { ...base, issue: invalidLabels },
    { ...base, issue: invalidAssignees }, { ...base, issue: invalidClosedAt },
  ];
  for (const payload of invalidIssues)
    assert.equal(githubRequest("issues", payload).statusCode, 400);

  const invalidPullRequests = [
    { ...pullRequest, head: null }, { ...pullRequest, base: { ref: "main" } },
    { ...pullRequest, merged: "false" }, { ...pullRequest, merged_at: undefined },
  ];
  for (const payload of invalidPullRequests)
    assert.equal(githubRequest("pull_request", { ...base, pull_request: payload }).statusCode, 400);

  const invalidComments = [
    { ...comment, body: null }, { ...comment, user: null },
    { ...comment, path: 42 }, { ...comment, position: "2" },
  ];
  for (const payload of invalidComments) {
    assert.equal(githubRequest("issue_comment", { ...base, issue, comment: payload }).statusCode, 400);
    const review = { ...base, pull_request: pullRequest, comment: payload };
    assert.equal(githubRequest("pull_request_review_comment", review).statusCode, 400);
  }
  const wrongParent = { ...base, issue, comment };
  assert.equal(githubRequest("pull_request_review_comment", wrongParent).statusCode, 400);
  const invalidMarker = { ...base, issue: { ...issue, pull_request: null }, comment };
...
test("validation preserves open actions, extra metadata, and nullable review positions", () => {
  const payload = {
    action: "future_action", repository, sender: user,
    pull_request: pullRequest, comment: { ...comment, position: null },
    installation: { id: 99 },
  };
